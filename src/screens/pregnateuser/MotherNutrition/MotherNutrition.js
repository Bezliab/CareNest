...
const HEALTH_GUIDELINES = {
  ...
  subheader:
    'A guide to the key vitamins, minerals, and dietary choices necessary for a healthy pregnancy.',
  keyNutrients: [
    {
      ...
      color: '#5cb85c', // Green
      details:
        'Crucial for preventing neural tube defects in the baby, especially during the first trimester. Recommended intake is usually 400-800 mcg daily. Sources include leafy greens, fortified cereals, and citrus fruits.',
    },
    {
      title: 'Iron',
      icon: '🥩',
      color: '#d9534f', // Red
      details:
        'Necessary to support the increased blood volume (up to 50% more!) and prevent anemia. Iron deficiency can cause fatigue and lead to preterm birth. Pair iron-rich foods (lean meats, beans) with Vitamin C for better absorption.',
    },
    {
      ...
      color: '#0275d8', // Blue
      details:
        "Calcium helps build the baby's bones and teeth. If intake is insufficient, the body will draw calcium from the mother's bones. Vitamin D is essential for calcium absorption. Sources: dairy, fortified juices, fatty fish, and sunlight.",
    },
    {
      ...
      color: '#f0ad4e', // Orange
      details:
        "Vital for the optimal development of the baby's brain, eyes, and nervous system. Consume low-mercury fish (like salmon) or use high-quality prenatal supplements to meet the recommended daily intake.",
    },
  ],
  dietaryTips: [
  ...
  return (
    <View style={styles.safeArea}>
      <ScrollView contentContainerStyle={styles.scrollViewContent}>
        ...
        {/* Header */}
        <Text style={styles.header}>{HEALTH_GUIDELINES.header}</Text>
        <Text style={styles.subheader}>{HEALTH_GUIDELINES.subheader}</Text>

        {/* Key Nutrients Section */}
        {HEALTH_GUIDELINES.keyNutrients.map((nutrient, index) => (
          <View key={index} style={styles.nutrientCard}>
            <View
              style={[styles.titleBar, { backgroundColor: nutrient.color }]}
            >
