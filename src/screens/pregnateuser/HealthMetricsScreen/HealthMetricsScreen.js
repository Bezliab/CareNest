...
const HealthMetricsScreen = () => {
  const { latestHealthData, healthHistory, loading, refreshHealthData } = useHealth();
      ...
      {latestHealthData ? (
        <View style={styles.currentData}>
          ...
          {latestHealthData.heartRate && (
            <Text style={styles.metric}>Heart Rate: {latestHealthData.heartRate} BPM</Text>
          )}
          
          {latestHealthData.bloodPressure && (
            <Text style={styles.metric}>Blood Pressure: {latestHealthData.bloodPressure}</Text>
          )}
          
          {latestHealthData.weight && (
            <Text style={styles.metric}>Weight: {latestHealthData.weight} kg</Text>
          )}
          
          {latestHealthData.temperature && (
            <Text style={styles.metric}>Temperature: {latestHealthData.temperature} °C</Text>
          )}
          
          {latestHealthData.timestamp && (
            <Text style={styles.timestamp}>
              Last updated: {new Date(latestHealthData.timestamp?.toDate()).toLocaleString()}
            </Text>
          )}
        </View>
      ) : (
